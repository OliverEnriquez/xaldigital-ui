...
const Product = () => {
  ...
  const submitValue = () => {
    ...
    axios
      ...
      .then((response) => {
        if (quantity > 0 && idProduct === null) {
          addPurchase(purchaseProduct, response.data);
        }
        updateImage();
        handleClose();
        getProducts();
  ...
  const addPurchase = (purchaseProduct, id) => {
    axios
      .post(
  ...
  const editProduct = (
    ...
    setFileDir("http://localhost:8080/get/image/" + name);
      ...
      <Row>
        <Col xs="12" md="12" lg="12">
          ...
          <Card>
            ...
            <CardBody className="">
              <Input id="name" name="name" placeholder="Buscar" type="name" />
              <Table bordered>
                <thead>
                  <tr>
                    <th>SKU</th>
